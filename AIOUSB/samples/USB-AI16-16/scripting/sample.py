...
if result == AIOUSB.AIOUSB_SUCCESS:
    ...
    print "Error '%s' attempting to read ground counts\n" % ( AIOUSB.AIOUSB_GetResultCodeAsString( abs(result) ) )
...
for seconds in range(100):
    ...
    if result == AIOUSB.AIOUSB_SUCCESS:
        print "  %lu bytes remaining" % ( bb.bytes_remaining )
        if bb.bytes_remaining == 0:
            break
    else:
        print  "Error '%s' polling bulk acquire progress" % ( AIOUSB.AIOUSB_GetResultCodeAsString( abs(result) ))
        break
