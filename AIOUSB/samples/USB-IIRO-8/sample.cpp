...
int main( int argc, char *argv[] ) {
	...
	try {
       	deviceManager.open();
		cout
			<< "USB-IIRO-8 sample program version: " << VERSION << "\n"
			...
			<< "  This program demonstrates controlling a USB-IIRO-8 device on\n"
			<< "  the USB bus. For simplicity, it uses the first such device found\n"
			<< "  on the bus.\n";
       	deviceManager.printDevices();
       	USBDeviceArray devices = deviceManager.getDeviceByProductID( USB_IIRO_8 );
		if( devices.size() > 0 ) {
			...
			if( DEMO_EEPROM_WRITE ) {
				...
				string message = "USB-IIRO-8 sample program";
			...
			cout << "No USB-IIRO-8 devices found on USB bus" << endl;
       	deviceManager.close();
	} catch( exception &ex ) {
